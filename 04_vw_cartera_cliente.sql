...
(select cf.numero_factura as Factura, cf.tipo_factura as Tipo, cf.fecha_factura ,
cf.mes_comisiones as Mes, ano_comision as perido,c.cia as cia, cf.monto_comisiones as Facturado,
cf.retenciones_comisiones as Retenciones,cf.cobrada as Depositada,
cf.fecha_carga_factura as Ultima_actualizacion, cf.pto_venta as Pto_venta, cf.titular  
...
-- suma de facturas por cia 
create or replace view vw_adm_sum_fac_cia as
SELECT cia,
       SUM(Retenciones) AS Total_Retenciones
	           SUM(vw_adm_fac_tot.Facturado) AS Total_Facturado,
        SUM(vw_adm_fac_tot.Retenciones) AS Total_Retenciones,
        (SUM(vw_adm_fac_tot.Facturado) - SUM(vw_adm_fac_tot.Retenciones)) AS Bruto
FROM vw_adm_fac_tot
GROUP BY cia
ORDER BY total_retenciones desc;

SAVEPOINT vw_adm_sum_fac_cia;
...
SAVEPOINT vw_adm_fac_cob;

--- vw de facturas del ultimo mes 
CREATE or REPLACE VIEW vw_adm_sum_fac_ult_periodo AS
    SELECT 
        vw_adm_fac_tot.cia AS cia,
        SUM(vw_adm_fac_tot.Facturado) AS Total_Facturado,
        SUM(vw_adm_fac_tot.Retenciones) AS Total_Retenciones,
        SUM(vw_adm_fac_tot.Facturado) - SUM(vw_adm_fac_tot.Retenciones) AS Diferencia
    FROM
        vw_adm_fac_tot
    WHERE
        (vw_adm_fac_tot.perido, vw_adm_fac_tot.mes) = (
            SELECT
                MAX(perido),
                MAX(mes)
            FROM
                vw_adm_fac_tot
            WHERE
                perido = YEAR(CURRENT_DATE) -- Obtener el año actual
        )
    GROUP BY vw_adm_fac_tot.cia
    ORDER BY Total_Retenciones DESC;

-- vw de facturas NO cobradas
CREATE or REPLACE VIEW vw_adm_fac_pend AS
...
-- vw estado_poliza
CREATE or REPLACE VIEW vw_t_estado_poliza AS
SELECT 
    id_estado_poliza as id_estado,
    estado_poliza as estado
    from estado_poliza
    ORDER BY estado ;


SAVEPOINT vw_t_estado_poliza;

-- vw estado_Poliza Vencida
CREATE or REPLACE VIEW  vw_polizas_vencida AS
    SELECT 
        vw_polizas.productor AS productor,
        vw_polizas.cia AS cia,
        vw_polizas.ramo AS ramo,
        vw_polizas.id_poliza AS poliza,
        vw_polizas.nom_apell AS cliente,
        vw_polizas.id_dni AS dni,
        vw_polizas.descrip AS descripcion,
        vw_polizas.fecha_alta AS Alta,
        vw_polizas.fecha_baja AS Baja,
        vw_polizas.estado_poliza AS Estado,
        vw_polizas.id_productor AS id_productor,
        vw_polizas.id_cia AS id_cia,
        vw_polizas.id_ramo AS id_ramo,
        vw_polizas.id_forma_pago_clientes AS id_forma_pago_clientes	,
        vw_polizas.cbu_tc AS cbu_tc,
		vw_polizas.cantidad_cuotas AS cantidad_cuotas,
        vw_polizas.id_ciclo_facturacion AS id_ciclo_facturacion
        

    FROM
        vw_polizas
    WHERE
        (vw_polizas.id_estado_poliza = 6)
    ORDER BY vw_polizas.fecha_modificacion_polizas
;
...
CREATE OR REPLACE VIEW cartera_cliente.vw_polizas_avencer as 
    ...
    from
        cartera_cliente.vw_polizas
    where
      (vw_polizas.id_estado_poliza = 11)
    ORDER BY vw_polizas.fecha_modificacion_polizas
;	


-- vw para iterar con polizas vigentes
CREATE OR REPLACE VIEW cartera_cliente.vw_polizas_vigentes as
    SELECT  * from vw_polizas
    where id_estado_poliza = 1 
    or id_estado_poliza = 10 
    or  id_estado_poliza = 6 
    or id_estado_poliza = 11
;

SAVEPOINT vw_polizas_vigentes;


-- vw para iterar con polizas vigentes
CREATE OR REPLACE VIEW cartera_cliente.vw_polizas_vigentes as
    SELECT * FROM vw_polizas_vigentes 
    WHERE sa_ajustado_dolar = 1
    ORDER BY = fecha_modificacion_polizas;


-- vw ajustadas a dolar  
CREATE OR REPLACE VIEW cartera_cliente.vw_poliza_sadolar as
	SELECT pv.cia, pv.ramo, pv.id_poliza, pv.id_dni, pv.nom_apell, pv.descrip, pv.fecha_baja, pv.estado_poliza,
		ct_cl.utlimo_contacto_cliente, ct_cl.comentarios, ct_cl.vencimiento_contacto_cliente
		FROM cartera_cliente.vw_polizas_vigentes pv
        left join cartera_cliente.contacto_cliente ct_cl on (ct_cl.id_poliza= pv.id_poliza )
		where sa_ajustado_dolar = 1
        order by utlimo_contacto_cliente asc;


SAVEPOINT vw_poliza_sadolar;
